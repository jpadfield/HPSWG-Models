...
| `sample_taking` | v1.6 | `S2: Sample Taking` | Stable; renamed from `sample_taking_event` |
...
The sample model (`sample_v1_8.tsv`) is the primary location for working through the dual-layer structure. Once finalised, all dependent models currently using the `E22/S13: Heritage Sample` placeholder (`sample_taking`, `sample_modification`, `sample_observation`, `sample_splitting`) should be updated consistently in a single pass.
...
Priority order for next batch: `sampling_activity`, `sample_taking`, `sample_site`, `sample`.
